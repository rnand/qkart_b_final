...
const addProductToCart = async (user, productId, quantity) => {
  ...
  if(!cart){
    try{
      ...
      throw new ApiError(
        httpStatus.INTERNAL_SERVER_ERROR, "User cart creation failed since the user already has a cart"
      );
    }
  }
  ...
  if(cart === null){
    throw new ApiError(
      httpStatus.INTERNAL_SERVER_ERROR, "User does not have a cart"
    );
  }

  //find the index of cartitem
  let productIndex = -1;
  ...
  //if the product is not in cart, add it, else throw error
  if (productIndex == -1) {
